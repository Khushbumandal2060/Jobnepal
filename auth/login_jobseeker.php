...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['email']) && isset($_GET['password'])) {
    ...
    try {
        ...
        $stmt = $pdo->prepare("SELECT * FROM users WHERE email = ? AND role = 'job_seeker'");
        $stmt->execute([$email]);
        $user = $stmt->fetch();
        ...
        if ($user && password_verify($password, $user['password'])) {
            ...
            $stmt = $pdo->prepare("SELECT id, name FROM job_seekers WHERE user_id = ?");
            $stmt->execute([$user['id']]);
            $jobSeeker = $stmt->fetch();
            $_SESSION['user_name'] = $jobSeeker['name'];

            $_SESSION['loggedin'] = true;

            $_SESSION['message'] = [
                'type' => 'success',
                'text' => "Login successful! Welcome back, " . htmlspecialchars($jobSeeker['name'])
            ];
